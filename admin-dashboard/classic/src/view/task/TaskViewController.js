Ext.define('Admin.view.task.TaskViewController', {
   ...
   // taskGridFind: function(btn){
	  ...
	  //  var userName=Ext.getCmp('taskCondition').items.getAt(1).getValue();
	  //  var operation=Ext.getCmp('taskCondition').items.getAt(3).getValue();
	  //  var beginTime=null;
	  //  var endTime=null;
	  //  beginTime=Ext.getCmp('taskCondition').items.getAt(5).getValue();

	  //  if(beginTime&&beginTime.length!=0){
			// endTime=Ext.Date.add(Ext.getCmp('taskCondition').items.getAt(7).getValue(), Ext.Date.DAY,1);
	  //  } else {
			// endTime=Ext.getCmp('taskCondition').items.getAt(7).getValue();
	  //  }
	  //  Ext.Ajax.request({ 
