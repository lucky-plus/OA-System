...
//    }
    ...
    fbPromise.then(function (fb) {
        ...
        Object.keys(apiBtns).forEach(function (action) {
            console.debug('ElementById:' + action);
        ...
        //        });
        $('.pull-right .btn-group').append('<button type="button" id="send" class="btn btn-success send">发送</button>');
						
        $('.pull-right .btn-group').children('button').eq(3).remove();

		$('.pull-right .frmb-control .icon-button').remove();
		$('.pull-right .frmb-control .icon-hidden').remove();
		$('.pull-right .frmb-control .icon-starRating').remove();
		$('.pull-right .frmb-control .input-set-control').remove();
		
        function sendToHost(urld, content) {
            $.ajax({
                url: urld,
                type: "POST",
                contentType: 'application/json;charset=utf-8', //设置请求头信息
                dataType: "json",
                data: JSON.stringify(content), //将Json对象序列化成Json字符串，JSON.stringify()原生态方法
        ...
        function sendFunction() {
            sendToHost("/submitUserList_1", JSON.parse(formBuilder.actions.getData('json', true)));
        ...
        document.getElementById('send-to').addEventListener('click',
            function () {
                sendFunction();
            });
    });
